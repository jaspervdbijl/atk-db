...
public class Query<T extends AbstractAtkEntity, O> {
    ...
    private String getProcessedSql(Filter filter) {
        ...
        StringBuilder returnSql = new StringBuilder("select " + star + " " + split.toString(" "));
    ...
    private int findIndexOfParamToReplace(int i, String sql) {
        int lastOccurenceIndex = 0;
        for (int index = 0; index <= i; index++) {
            lastOccurenceIndex = sql.indexOf("?", lastOccurenceIndex) + 1;
        }
        return lastOccurenceIndex - 1;
    }
    ...
    public void getAll(DataSource dataSource, CallOne<T> iterate, int limit, String sql, Object... params) {
        getAll(dataSource, new Filter(sql, params), iterate, limit);
    }
    ...
    public void getAllFromResource(Connection connection, CallOne<T> iterate, int limit, String resource, Object...
            params) {
    ...
    public static void main(String[] args) {
        StringBuilder input = new StringBuilder("select latitude,        longitude,        a_type,        notes,        c_alarms.location_description,        occur_datetime,        c_alarms.driver_id,        c_alarms.trip_id,        speed from c_alarms , c_dev_locations where vehicle_id = ? and c_dev_locations.id = dev_location_id and a_type in (?) order by occur_datetime desc");

        System.out.println(input.charAt(299));
        System.out.println(input);
        System.out.println(input.replace(299, 300, "?,?,?"));
    }
